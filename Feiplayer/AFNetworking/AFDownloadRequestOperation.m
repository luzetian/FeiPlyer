...
- (void)setDownloadProgress:(NSProgress * __autoreleasing *)progress
{
    if (progress) {
...
- (void)connection:(NSURLConnection *)connection didReceiveData:(NSData *)data  {
    ...
    dispatch_async(self.progressiveDownloadCallbackQueue ?: dispatch_get_main_queue(), ^{
        self.progress.totalUnitCount = self.totalContentLength;
        self.progress.completedUnitCount = self.totalBytesReadPerDownload + self.offsetContentLength;
    });
}
...
+ (NSString *)cacheFolder {
    ...
    if (!cacheFolder) {
        NSString *cacheDir = [self documentPath];
