...
- (void)viewDidLoad {
 ...
 //   [self.navigationController loadThemeIphone];
...
{
    [self.settingsStore synchronize];
    //[self toggDrawerSideRightOrLeft:YES];
}

- (void)showMenu:(UIBarButtonItem *)button
{
    [self.settingsStore synchronize];
   // [self toggDrawerSideRightOrLeft:NO];
}
/*
- (void)toggDrawerSideRightOrLeft:(BOOL)isRight
{
    id viewController = self.navigationController.mm_drawerController;
    if (![viewController isKindOfClass:[LTMainViewController class]]) {
        return;
    }
    LTMainViewController *drawerController = viewController;
    if (isRight) {
